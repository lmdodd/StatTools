...
makeLTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false,double err=0.2)
{
	setTDRStyle();
	...
	if(s==3) {
		TH1F * ZH = (TH1F*)(f->Get(dir+"/Zprime1200A400h"));
		TH1F * AH = (TH1F*)(f->Get(dir+"/Zprime600A400h"));
		TH1F *zh = ZH;
		TH1F *ah = AH;
		zh->Scale(1);//FIXME
		ah->Scale(1);//FIXME
