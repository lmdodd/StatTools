...
void makeLTauStack(TString name,TString file,TString dir,int s,TString labelX,TString units = "GeV",bool left=false,TString channel = "",TString json = "Golden",bool log = false,bool dndm=false,bool doRatio = false, int blind=900, bool doFit=false)
{
    ...
    float zvvError = zjftErr->GetBinContent(1);
    zvvError = 0.5;
    //if (channel=="#tau_{h}#tau_{h}") zvvError=0.5;
    ...
    //if (channel == "#tau_{e}#tau_{h}"||channel == "#tau_{#mu}#tau_{h}"){
        ...
        if (isnan(zjftError)) zjftError = 0.0;
        if (isnan(zlftError)) zlftError = 0.0;
    ...
    errorBand->Add(SMH);
    errorBand->Add(ZTT);
    if (channel!="#tau_{h}#tau_{h}") {errorBand->Add(zvv); errorBand->Add(QCD);}
    ...
    if(s==3) for (int i=0; i<data->GetNbinsX();i++){
    ...
    if(channel == "#tau_{h}#tau_{h}") l->AddEntry(ZEE,"Z#rightarrow other","F");
